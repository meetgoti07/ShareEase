...
import RentFilter from "@/pages/rent/Property/RentFilter.tsx";
...
export const Rent = () => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                ...
                console.log(data);
                setProperties(data);
                setFilteredProperties(data.filter(property => property));
            } catch (error) {
    ...
    const handleFilterChange = ({ rentRange, locations, furnished, sortBy, isAvailable }) => {
        ...
        switch (sortBy) {
            case "rent-asc":
                filtered.sort((a, b) => (a.rent_per_month) - (b.rent_per_month));
                break;
            case "rent-desc":
                filtered.sort((a, b) => (b.rent_per_month) - (a.rent_per_month));
                break;
            case "newest":
